...
public class OreSim extends Module {
    ...
    private final Setting<Boolean> baritone = sgGeneral.add(new BoolSetting.Builder()
            .name("Baritone")
            .description("将Baritone矿石位置设置为模拟的位置。")
            .defaultValue(false)
            .build()
    );

    public OreSim() {
        super(MeteorRejectsAddon.CATEGORY, "矿石模拟", "X射线增强版。");
        SettingGroup sgOres = settings.createGroup("矿石");
    ...
    public void onActivate() {
        if (Seeds.get().getSeed() == null) {
            error("未找到种子。要设置种子，请执行.seed <seed>");
            this.toggle();
        }
        reload();
    }
