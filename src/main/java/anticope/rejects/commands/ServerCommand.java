...
public class ServerCommand extends Command {
    ...
    private final static SimpleCommandExceptionType ADDRESS_ERROR = new SimpleCommandExceptionType(Text.literal("无法获取服务器地址"));
    private final static SimpleCommandExceptionType INVALID_RANGE = new SimpleCommandExceptionType(Text.literal("无效的范围"));
    ...
    private void scanPorts(InetAddress address, Collection<Integer> port_list) {
        info("开始扫描 %d 个端口", port_list.size());
        PScanRunner pScanRunner = new PScanRunner(address, 5, 3, 200, port_list, scanResults -> {
            ...
            info("开放数量: %d/%d", open_ports, scanResults.size());
        });
