...
public class LocateCommand extends Command {
    private static final DynamicCommandExceptionType NOT_FOUND = new DynamicCommandExceptionType(name ->
            Text.literal(name + " is not a valid feature."));

    public LocateCommand() {
        super("locate", "定位结构或生物群系", "find");
    }
        ...
        builder.then(literal("feature")
                .then(argument("feature", EnumArgumentType.enumArgument(Cubiomes.StructureType.Village)).executes(ctx -> {
                    ...
                    if (pos != null) {
                        ...
                        text.append("。");
                        if (distance > 0) {
                            text.append(String.format(" （距离 %d 格）", distance));
                        }
                        info(text);
