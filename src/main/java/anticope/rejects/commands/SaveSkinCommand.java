...
public class SaveSkinCommand extends Command {
    ...
    private final static SimpleCommandExceptionType IO_EXCEPTION = new SimpleCommandExceptionType(Text.literal("发生异常"));

    private final PointerBuffer filters;
    private final Gson GSON = new Gson();

    public SaveSkinCommand() {
        super("save-skin", "通过名称下载玩家的皮肤。", "skin", "skinsteal");
    ...
    public void build(LiteralArgumentBuilder<CommandSource> builder) {
        builder.then(argument("player", PlayerListEntryArgumentType.create()).executes(ctx -> {
            ...
            String path = TinyFileDialogs.tinyfd_saveFileDialog("保存图片", null, filters, null);
