...
public class ServerFinderScreen extends WindowScreen implements IServerFinderDoneListener {
    ...
    public void initWidgets() {
        add(theme.label("这将搜索与您输入的IP地址相似的服务器"));
        add(theme.label("您在下面的字段中键入的IP"));
        add(theme.label("找到的服务器将被添加到您的服务器列表中"));
        WTable table = add(new WTable()).expandX().widget();
        table.add(theme.label("服务器地址:"));
        table.add(ipBox).expandX();
        table.row();
        table.add(theme.label("最大线程数:"));
        table.add(maxThreadsBox);
        table.row();
        table.add(theme.label("扫描端口"));
        table.add(scanPortsBox);
        table.row();
        table.add(theme.label("版本:"));
    ...
    @Override
    public void tick() {
        searchButton.set(state.isRunning() ? "取消" : "搜索");
        if (state.isRunning()) {
            ipBox.setFocused(false);
            maxThreadsBox.set(maxThreads);
        }
        stateLabel.set(state.toString());
        checkedLabel.set("已检查: " + checked + " / " + targetChecked);
        workingLabel.set("可用: " + working);
    ...
    public void onServerDone(ServerPinger pinger) {
        ...
        if (pinger.isWorking()) {
            if (!isServerInList(pinger.getServerIP()) && filterPass(pinger.getServerInfo())) {
                synchronized (serverFinderLock) {
                    working++;
                    multiplayerScreen.getServerList().add(new ServerInfo("服务器发现 #" + working, pinger.getServerIP(), ServerInfo.ServerType.OTHER), false);
