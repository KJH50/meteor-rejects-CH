...
public class CleanUpScreen extends WindowScreen {
    ...
    public void initWidgets() {
        ...
        table.row();
        table.add(theme.label("未知主机:")).widget().tooltip = "";
        table.add(removeUnknown).widget();
        table.row();
        table.add(theme.label("过时服务器:"));
        table.add(removeOutdated).widget();
        table.row();
        table.add(theme.label("Ping失败:"));
        table.add(removeFailed).widget();
        table.row();
        table.add(theme.label("\"服务器发现\"服务器:"));
        table.add(removeGriefMe).widget();
        table.row();
        table.add(theme.label("全部:")).widget().color = new Color(255, 0, 0);
        table.add(removeAll).widget();
        table.row();
        table.add(theme.label("重复项:"));
        table.add(removeDuplicates).widget();
        table.row();
        table.add(theme.label("重命名所有服务器:"));
        table.add(rename).widget();
        table.row();
        table.add(theme.button("执行!")).expandX().widget().action = this::cleanUp;
    }
